...
class SwinTransformerBlock(nn.Module):
    ...
    Args:
        ...
        num_heads (int): Number of attention heads.
        window_size tupple[int]: Window size.
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=(1,4), shift_size=0,
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, W, C)
        x_0 = x[:, :, 0::4, :]  # B H/2 W/2 C
        x_1 = x[:, :, 1::4, :]
        x_2 = x[:, :, 2::4, :]
        x_3 = x[:, :, 3::4, :]# B H/2 W/2 C
        
        #merge the windows with (1,4)
        if W//4 == 2:
            results = torch.empty(B, H//1, W//4, 4*C).cuda()
            t = torch.cat((x_0[:, :H//2], x_1[:, :H//2], x_2[:, :H//2], x_3[:, :H//2]), -1)
            t_ = torch.cat((x_0[:, H//2:], x_1[:, H//2:], x_2[:, H//2:], x_3[:, H//2:]), -1)
            results[:, 0::2, :, :] = t
            results[:, 1::2, :, :] = t_
        else:
            results = torch.cat([x_0, x_1, x_2, x_3], -1)

        x = results.view(B, -1, 4 * C)  # B H/1*W/4 4*C 
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 shift_size=(0, 0) if (i % 2 == 0) else (0, max(window_size) // 2),
                                 mlp_ratio=mlp_ratio,
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, radius_cuts=16, azimuth_cuts = 64, in_chans=3, num_classes=1000,
                 ...
                 window_size=(1, 4), mlp_ratio=4., qkv_bias=True, qk_scale=None,
