...
class FinalPatchExpand_X4(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, n_rad = 8, patch_size = 2,  norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        self.dim_scale = dim_scale
        # ks = int(np.sqrt(n_rad*2))
        self.rad_cut = input_resolution[0]
        k_s = (n_rad*2)//patch_size
        # self.nrad = 70
        # self.nazimuth = 12
        # self.expand = nn.Linear(dim, 4*dim, bias=False)

        # self.m = nn.ConvTranspose2d(dim, dim, kernel_size = (3, 3), stride=(3, 3))
        # self.n = nn.ConvTranspose2d(dim, dim, kernel_size = (2, 2), stride=(2, 2))
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, n_radius = 2, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.subdiv = (patches_resolution[0]*n_rad, patches_resolution[1]*n_rad)
...
class Radial_curve_cds(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=128, patch_size=4, n_radius = 3, in_chans=3, num_classes=1000,
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, n_radius = n_radius, in_chans=in_chans, embed_dim=embed_dim,
...
if __name__=='__main__':
    ...
    model = Radial_curve_cds(img_size=128,
                        patch_size=2,
                        in_chans=3,
                        num_classes=200,
                        embed_dim=96,
