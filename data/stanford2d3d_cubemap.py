...
normalize= transforms.Normalize(
            ...
            std= std )
#normalize = None
class Stanford_cubemap(Dataset):
    def __init__(self, base_dir, split, grp, n_rad,  xi=0.0, model= "spherical", img_size = 128, fov = 170, high=0.0, low=0.35, transform=None):
        ...
        if split == 'train':
            ...
            # print(base_dir + '/12NN_2_64_concentric_test.pkl')
            ...
            with open(base_dir + '/test_sem.pkl', 'rb') as f:
                sem = pkl.load(f)
                sem = sem
            # with open(base_dir + '/12NN_' + str(n_rad) + '_' +  str(img_size) + '.pkl', 'rb') as f:
            #     dist = pkl.load(f)
    ...
    def __getitem__(self, idx):
        ...
        # Image.fromarray((image).astype(np.uint8)).save("fish.png")
        image = cubemap(image, self.n_rad, fov, xi, self.img_size, 1)
        ...
        # label = cubemap(label, fov, xi, self.img_size, 0)
